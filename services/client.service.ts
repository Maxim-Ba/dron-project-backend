...
class ClientServices {
  async createClient(name:string,inn:number|null = null, phone:number|null = null) {
    try {
      const result = await pool.query('INSERT INTO client (name, inn, phone) VALUES ($1, $2, $3)',
        [name, inn, phone])
  ...
  async getClients(): Promise<any> {
    try {
      ...
      const {rows} = await pool.query('SELECT * FROM client ORDER BY name ASC')
      return rows;
  ...
  async getClient(id :number): Promise<any> {
    try {
      ...
      const {rows} = await pool.query('SELECT * FROM client WHERE id = $1 ORDER BY name ASC', [id])
      return rows;
